...
**随时新建配置**：菜单内按 `N` 可再次进入同一配置向导 —— DeepSeek 官方配置默认创建 V4-Flash 模板，模板名称可自定义（回车用默认名，重名时提示覆盖或改名），同时写入官方模型目录 `models.json`（含 V4-Flash / V4-Pro，可在 Codex 内用 `/model` 或 `--model` 切换）；也可打开目录导入自己的模板。已有模板与状态不受影响。
