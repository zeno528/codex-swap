...
.githooks/            pre-commit（递增 VERSION + 一次写入 CHANGELOG）+ prepare-commit-msg（兜底写）+ post-commit（兜底 amend）+ lib.sh（共享函数）
```
...
- `pre-commit` hook（`.githooks/pre-commit`，`core.hooksPath` 指向 `.githooks`）：改动发布物相关文件（`linux/*`、`windows/src/*`、`windows/bin/*`、安装器、`scripts/build.sh`）时自动递增 `VERSION` 的 patch（99 进位，0.0.99 → 0.1.0）；纯文档/CI/元数据改动不递增；检测到工作区 `VERSION` ≠ HEAD 时跳过（发版手动升 minor/major 不重复递增）；递增后同步三处源码版本（文件缺失时跳过）；若能从 git 进程命令行拿到提交消息（`-m`/`--message`/`-F`/`--file`/`--amend`，解析 `/proc/$PPID/cmdline`），CHANGELOG 条目一次写入并随本次提交入库（pre-commit 阶段 `git add` 有效）
- `prepare-commit-msg` hook（`.githooks/prepare-commit-msg`）：编辑器交互提交等 pre-commit 拿不到消息的兜底 —— 写 CHANGELOG 条目（版本号 + 日期 + 消息首行）；非发布物提交不写；同消息已存在则跳过（幂等）；该阶段 git 提交树已冻结（`git add` 不进本次提交，实测），条目由 post-commit 并入
- `post-commit` hook（`.githooks/post-commit`）：仅兜底场景触发 —— 检测到 CHANGELOG 有未提交改动时自动 `git commit --amend --no-verify --no-edit` 并入；`--no-verify` 防 pre-commit 重复递增 VERSION，amend 后的 post-commit 自检无改动不循环
- 发布物判定与 changelog 写入共用 `.githooks/lib.sh` 的 `is_release_change` / `get_commit_msg` / `write_changelog_entry`
...
- 临时 git 仓库模拟三轮提交：`-m` 发布物提交 → 递增 + changelog 一次写入（断言无 amend、条目在 HEAD、工作区干净）；非发布物提交 → 不递增不写；`-F` 消息文件场景 → 解析一次写入同样无 amend
- 改动 `.githooks/*` 后必须跑 `scripts/test-hooks.sh`

### CI（push 自动跑）

- `ci.yml`：Windows 跑 pwsh 测试 + Ubuntu 跑 bash 语法/冒烟/hooks 测试
