...
#   --package  构建 dist/ 并产出 codex-switch-windows.zip / codex-switch-linux.zip（默认）
set -euo pipefail
...
case "${1:---package}" in
    ...
    --package)
        ...
        cp "$VFILE" "$ROOT/dist/codex-switch"
        sed -i "s/@VERSION@/$VERSION/g" "$ROOT/dist/src"/*.psm1 "$ROOT/dist/src"/*.psd1 "$ROOT/dist/codex-switch"
        grep -rq '@VERSION@' "$ROOT/dist" && { echo "错误: dist 仍残留占位符 @VERSION@" >&2; exit 1; }
        (cd "$ROOT/dist" && zip -qr "$ROOT/codex-switch-windows.zip" src bin && zip -qr "$ROOT/codex-switch-linux.zip" codex-switch)
        echo "已构建 v$VERSION:"
        echo "  codex-switch-windows.zip（src/ bin/）"
        echo "  codex-switch-linux.zip（codex-switch）"
        ;;
