...
psm1_ver="$(sed -n "s/^\$script:ScriptVersion[[:space:]]*=[[:space:]]*'\([^']*\)'.*/\1/p" windows/src/codex-swap.psm1)"
psd1_ver="$(sed -n "s/^[[:space:]]*ModuleVersion[[:space:]]*=[[:space:]]*'\([^']*\)'.*/\1/p" windows/src/codex-swap.psd1)"
linux_ver="$(sed -n 's/^VERSION="\([^"]*\)".*/\1/p' linux/codex-swap)"
